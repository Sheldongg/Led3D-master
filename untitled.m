...
clc;
read_file = 'sample/data/depth/001_Kinect_FE_1DEPTH/01.png'; %读取文件
% file_x = read_file(:,1);
% file_y = read_file(:,2);
% file_z = read_file(:,3);
% % set(figure,'name','点云');
...
% reSize=360;
% roi_face = imresize(depth_face,[reSize,reSize]);
...
file_x = pc_template(1,:);
file_y = pc_template(2,:);
file_z = pc_template(3,:);
% set(figure,'name','点云');
% plot3(file_x,file_y,file_z,'.')
[X,Y,Z] = griddata(file_x,file_y,file_z,linspace(min(file_x),max(file_x),200)',linspace(min(file_y),max(file_y),20),'cubic'); %插值
