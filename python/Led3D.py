...
def conv_1x1(data,nnum_filter,name):
    data = mx.sym.Convolution(data=data, num_filter=nnum_filter, kernel=(1, 1), stride=(1, 1), pad=(0, 0), #num_filter : int (non-negative), required Convolution filter(channel) number
...
def get_symbol(num_classes=509):
    ...
    # stage 1   128*128         every block has 3x3 convolution layers,BatchNormal,ReLU
    ...
    #MSFF feature map with max pooling,conact them in channel，actullay,conact and conc
    ...
    #SAV  attention weight into feature map
    ...
    dr = mx.sym.Dropout(data=s5_global_conv1,p=0.2,name='drop')  #Doupout avoid overfitting
    fc1 = mx.sym.FullyConnected(data=dr, num_hidden=num_classes, name='fc')

    return mx.sym.SoftmaxOutput(data=fc1, grad_scale=1, name='softmax') #guide train